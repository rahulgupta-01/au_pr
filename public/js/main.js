document.addEventListener('DOMContentLoaded', () => {
    ...
    // All logic is based on this date. Change this to see the site on a different day.
    const SIMULATION_DATE_STRING = '2025-08-01';

    // Reference "today" for all logic checks (progress bar, metrics, etc.).
    const todayForCalculations = new Date(SIMULATION_DATE_STRING + 'T00:00:00');

    // Used by the live clock to make it "tick".
    ...
    function initializeDashboard() {
        // Dashboard logic uses the global `todayForCalculations`
    }
    ...
    function initializeTimeline() {
        function renderTimeline(filter = 'all') {
            ...
            if (totalVisible === 0) {
                timelineEl.innerHTML = '<p style="padding: 20px; text-align: center;">No milestones for this phase.</p>';
                return;
            }
