document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================================================
    // 1.B GLOBAL HELPERS & DATA (used across multiple pages)
    // ==========================================================================
    ...
    const milestones = [
        { 
          id: 'student_visa_1_grant', 
          title: '🛂 First Student Visa Granted', 
          date: '2021-01-20', 
          phase: 'visa', 
          details: 'Granted the initial Student (subclass 500) visa, allowing me to formally study the Bachelor of Information Technology in Australia. This visa was valid until August 30, 2023.' 
        },
        { 
          id: 'student_visa_2_apply', 
          title: '📝 Applied for Visa Extension', 
          date: '2023-08-23', 
          phase: 'visa', 
          details: 'Applied for a new Student Visa to cover the extended course duration. A Bridging A visa was granted on this day to maintain my legal status while the application was processed.' 
        },
        { 
          id: 'student_visa_2_grant', 
          title: '✅ Student Visa Extension Granted', 
          date: '2023-10-23', 
          phase: 'visa', 
          details: 'The new Student (subclass 500) visa was granted, extending my stay until March 15, 2025, to allow for the completion of my degree.' 
        },
        { 
          id: 'visa_485_apply', 
          title: '📨 Applied for Graduate Visa (485)', 
          date: '2025-01-29', 
          phase: 'visa', 
    ...
    // ==========================================================================
    // 3. LIVE CLOCK (runs on all pages)
    // ==========================================================================
    ...
    // ==========================================================================
    // 4. DASHBOARD PAGE LOGIC (index.html)
    // ==========================================================================
    if (document.getElementById('points-breakdown')) {
        ...
        function loadState() {
            ...
            // If no saved state, create a default one
            const defaultState = { points: {}, costs: {} };
            getPointsData().forEach(p => {
                const isAchievedByDate = p.achievedDate && new Date() >= new Date(p.achievedDate);
                defaultState.points[p.id] = p.initial || isAchievedByDate;
            });
        ...
        function renderPoints() {
            ...
            elements.pointsBreakdown.innerHTML = pointsData.map(p => {
                ...
                return `<div class="points-item">
                            ...
                            <label for="check_${p.id}" class="item-label">${p.label} <span class="tooltip">(i)<span class="tooltiptext">${p.tooltip}</span></span></label>
            ...
            elements.pointsProgress.style.width = `${(currentTotal / targetTotal) * 100}%`;
            document.querySelectorAll('#points-breakdown .interactive-checkbox:not(:disabled)').forEach(box => {
                box.addEventListener('change', (e) => {
                    state.points[e.target.dataset.id] = e.target.checked;
                    saveState();
                    renderPoints();
                });
            });
        ...
        function renderCosts() {
            ...
            elements.costTracker.innerHTML = costData.map(c => {
                 ...
                 return `<div class="cost-item">
                            <input type="checkbox" class="interactive-checkbox" id="check_cost_${c.id}" data-id="${c.id}" ${state.costs[c.id] ? 'checked' : ''}>
                            <label for="check_cost_${c.id}" class="item-label">${c.label}</label>
            ...
            document.querySelectorAll('#cost-tracker .interactive-checkbox').forEach(box => {
                box.addEventListener('change', (e) => {
                    state.costs[e.target.dataset.id] = e.target.checked;
                    saveState();
                    renderCosts();
                });
            });
        ...
        function updateAlerts() {
            ...
            if (alerts.length === 0) {
                alerts.push({type: 'success', msg: 'All systems are green. You are on track!'});
            }
        ...
        function initializeResetButtons() {
            const handleReset = () => {
                const isConfirmed = confirm("Are you sure you want to reset all calculators to their default values? This action cannot be undone.");
                if (isConfirmed) {
                    localStorage.removeItem('prDashboardState');
                    location.reload();
                }
            };
    ...
    // ==========================================================================
    // 5. TIMELINE PAGE LOGIC (plan.html)
    // ==========================================================================
        ...
        function renderTimeline(filter = 'all') {
            ...
            timelineEl.innerHTML = `<div id="timeline-progress-fill" style="height: ${progressPercent}%"></div>` + milestones
                ...
                .map(m => {
                    ...
                    let statusClass = isCompleted ? 'completed' : 'future';
                    
                    return `<div class="milestone" data-phase="${m.phase}">
                                <div class="milestone-header" data-id="${m.id}">
                                    <div class="milestone-icon ${statusClass}"><i class="fas fa-${isCompleted ? 'check' : 'hourglass-start'}"></i></div>
                                    <div class="milestone-content">
                                        <div class="milestone-title">${m.title}</div>
                                        <div class="milestone-date">${mDate.toLocaleDateString('en-AU', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</div>
                                    </div>
                                </div>
    ...
    // ==========================================================================
    // 6. CONTACT PAGE LOGIC (contact.html)
    // ==========================================================================
